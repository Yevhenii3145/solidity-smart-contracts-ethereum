...
describe("Demo", function() {
    ...
    it("should allow owner to withdraw funds", async function() {
        ...
        await expect(() => tx).to.changeEtherBalance([demo,owner], [-amount, amount])
    })
    it("should not allow other account to withdraw funds", async function() {
        await sendMoney(other_addr)

        await expect(demo.connect(other_addr).withdraw(other_addr.address)).to.be.revertedWith("you are not an owner!")
    })
})